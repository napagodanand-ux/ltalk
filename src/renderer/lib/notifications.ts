...
export async function getNotificationSupport(): Promise<boolean> {
  ...
  if (electron?.notifications) {
    try {
      return await electron.notifications.permission();
    } catch {
      return false;
    }
  }
...
// notification to both trigger that prompt and prove the channel works.
export async function requestNotificationPermission(): Promise<boolean> {
  ...
  if (electron?.notifications) {
    ...
    try {
      await electron.notifications.send(
...
export async function notify({ title, body, tag }: NotifyOptions): Promise<void> {
  ...
  if (electron?.notifications) {
    try {
      await electron.notifications.send(title, body);
    } catch {
      /* best effort */
    }
    return;
